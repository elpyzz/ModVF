...
export function CreditsDisplay({ variant = 'dashboard' }: { variant?: Variant }) {
  ...
  const base = clsx(
    'inline-flex items-center gap-2 rounded-lg border border-brand-400/20 bg-brand-400/10 text-brand-400 transition-colors',
    variant === 'compact' ? 'px-3 py-1.5 text-sm' : 'px-4 py-2 text-sm',
    highlight && 'ring-1 ring-brand-400/30',
  )

  return (
    <div className={base}>
      <Coins className={variant === 'compact' ? 'h-3.5 w-3.5' : 'h-4 w-4'} />
      <span className="tabular-nums">{variant === 'compact' ? `${label} crédits` : `${label} crédits restants`}</span>
      {isZero ? (
        <Link
          to="/tarifs"
          className={clsx('ml-1 font-medium text-purchase underline-offset-2 hover:underline', variant === 'compact' && 'text-[11px]')}
        >
          Acheter
        </Link>
      ) : null}
